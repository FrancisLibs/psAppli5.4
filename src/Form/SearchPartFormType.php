...
class SearchPartFormType extends AbstractType
{
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        if ($options['afficher_organisation']) {
            $builder->add(
                'organisation',
                EntityType::class,
                [
        ...
        $builder->addEventListener(
            ...
            function (FormEvent $event) use ($options) {
                $form = $event->getData();
                if (is_null($form->organisation)) {
                    $form->organisation = ($options['organisation']);
                }
            }
        );
    }
